...
public class CategoryController {
    ...
    @GetMapping
    public ResponseEntity<List<CategoryDtoResponse>> list(){
        var list = categoryService.list();
        return ResponseEntity.ok(list.stream().map(category-> CategoryDtoResponse.builder()
                .id(category.getId())
    ...
    public ResponseEntity<CategoryDtoResponse> findCategoryById(
            @PathVariable("idCategory") Integer id
    ){
        var category = categoryService.findById(id);
        return ResponseEntity.ok(CategoryDtoResponse.builder()
                .id(category.getId())
                .name(category.getName())
                .build());
    }
    ...
    public ResponseEntity<List<CategoryDtoResponse>> findCategoryByName(
            @PathVariable String name
    ){
        var listCategory = categoryService.findByName(name);
        return ResponseEntity.ok(listCategory.stream().map(category -> CategoryDtoResponse.builder()
                .id(category.getId())
                .name(category.getName())
                .build()).toList());
    }
    ...
    @PostMapping
    public ResponseEntity<CategoryDtoResponse> save(
